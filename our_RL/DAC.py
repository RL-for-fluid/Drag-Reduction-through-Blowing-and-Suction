...
alpha_t_base=1e-2/50            # policy parameter
alpha_t = alpha_t_base       # policy parameter
num_train_episode = 20       # the number of episodes for fitting Q
epi_fin = 500
obs_std=0.1
the_std=0.01
...
#====  divied===# the and phi_imp_np are used!!!!!!!!!!!!!
...
the=np.zeros(dim_the)
#np.random.normal(loc=0,scale=1e-5,size=dim_the)
#the=np.random.normal(loc=0,scale=1e-3,size=dim_the)
...
#========= Acquisition of features =========# 
        t1=time.time()##########################################################
        obs_scale_np = (obs_np - obs_np.mean(0)) / obs_np.std(0) * obs_std
        the_scale_np = (the_np - the_np.mean(0)) / the_np.std(0) * the_std
        obs_the_np=np.concatenate([obs_scale_np, the_scale_np], 1)
        print ("std", obs_the_np.std(0))
        std_history.append(obs_the_np.std(0))
...
#==== Q fitting and policy parameter ====#      
        ...
        alpha_t=alpha_t_base-alpha_t_base/epi_fin * (i_episode-num_train_episode) #1e-4/TDerr*alpha_t_base
        if alpha_t < 0:
            alpha_t=0
        t2=time.time()##########################################################
        Qfit3_time=t2-t1##########################################################
    ...
    the_history.append(np.array(the))
...
#=== Saving experience and objective function ===#
    ...
    with open('the_history.pickle', 'wb') as f:
        pickle.dump(the_history,f)
